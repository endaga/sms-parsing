...
def get_rp_address(h):
	...
	return (rp_dest_address_type, strip_fs(reverse_byte_order(rp_dest_address)), h)
...
class RPDU:
	def __init__(self, rp_mti, rp_message_reference, rp_originator_address, rp_destination_address, user_data, rp_destination_address_type="81", rp_originator_address_type="81"):
		self.rp_mti = rp_mti
	...
	def toPDU(self):
		return ''.join([self.rp_mti, self.rp_message_reference, self.rp_originator_address, self.rp_destination_address, self.user_data])
	...
	def fromPDU(cls, rp_message):
		...
		(rp_originator_address_type, rp_originator_address, rp_message) = (get_rp_address(rp_message))
		(rp_dest_address_type, rp_dest_address, rp_message) = (get_rp_address(rp_message))
		rp_user_data = get_rp_user_data(rp_message)
		rp_user_data = rp_user_data[0][rp_user_data[1]:]
		return cls(rp_message_type, rp_message_reference, rp_originator_address, rp_dest_address, rp_user_data)
